...
| **SDK (v2 manifest-first)** | agents/tools/workflows/triggers declared in `app.yaml`; an agent is a `system_prompt` (zero-Python `promptFile`, or an `@agent(id)` class — NEVER `def execute()`/`AgentResult`); workflows + triggers are YAML; AI runs through the SDK tool-use loop (never a raw provider SDK) |
