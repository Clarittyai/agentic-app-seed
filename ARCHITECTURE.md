...
**Frontend must render 2 widget sizes (Apple standards)**:
...
export default function Widget({ size = 'large' }: WidgetProps) {
  ...
  if (size === 'small') {
    return (
      <div className="w-[170px] h-[170px] p-4 overflow-hidden rounded-3xl">
        {/* Small square widget: Primary metric + status */}
  ...
  return (
    <div className="w-[360px] h-[170px] p-4 overflow-hidden rounded-3xl">
      {/* Large wide rectangle: Stats grid + recent activity + actions */}
    </div>
  );
}
```
