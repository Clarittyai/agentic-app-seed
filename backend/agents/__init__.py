...
Agents are DECLARED in app.yaml#agents (schema lives there). An agent is a
system PROMPT, not an execute() method — the SDK's tool-use loop runs it.

Preferred: a zero-Python custom agent — set promptFile in app.yaml and write
...
class here and point app.yaml `handler:` at it:

    from claritty_sdk import agent, AgentContext, BaseAgent

    @agent(id="my-agent")                 # id ONLY; schema is in app.yaml
